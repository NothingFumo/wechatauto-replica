...
> 版本：1.0.0 ｜ 适用：微信 4.x Windows 客户端 ｜ 语言：Python 3.9+
...
区域采用**自绘渲染**（`MMUIRenderSubWindowHW`），对 UIAutomation / MSAA 完全
不暴露内容。因此本项目拆成两条技术路线：
...
- 主窗口：类名 `Qt51514QWindowIcon`，标题含「微信」；
- 渲染子窗口：类名 `MMUIRenderSubWindowHW`（自绘内容所在）；
- `_update_render_rect()` 读 `GetWindowRect` 得到渲染区，`_update_layout()`
  按比例换算各布局常量：
  - `SIDEBAR_RATIO=0.22` → 侧栏宽度 / 渲染区宽度；
...
- `msgs/`：`BaseMessage` 及子类（`TextMessage/ImageMessage/VideoMessage/
  ...
  PersonalCardMessage/SystemMessage`），`parse_msg()` 统一解析；
