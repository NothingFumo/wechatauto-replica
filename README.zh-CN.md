...
- **可操作的密钥报错**：`数据库无可用密钥` 报错前会自动向 stderr 输出三项内置诊断（Python 位数、逐个微信进程的内存读取权限、多账号目录与所选账号对比），随后提示运行 `python -m wechatauto.diagnose_keys` 并给出 keys.json 缓存路径；`_load_or_extract_keys` 在缺密钥时也向 stderr 输出缺失库清单与提取进度。
