...
- **New diagnostic tool**: `wechatauto/diagnose_keys.py` (`python -m wechatauto.diagnose_keys`, WeChat logged in) dumps library version, Weixin PIDs, data-dir/account detection, cached keys, fresh in-memory extraction results, and key verification — paste the output when reporting key-extraction failures.
